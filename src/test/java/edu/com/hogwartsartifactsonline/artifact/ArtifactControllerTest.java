...
class ArtifactControllerTest {
    ...
    void testFindArtifactByIdNotFound() throws Exception {
        ...
        //Given
        given(this.artifactService.findById("1250808601744904191")).willThrow(new ArtifactNotFoundException("1250808601744904196"));

        //When and then
        this.mockMvc.perform(get("/api/v1/artifacts/1250808601744904191").accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(false))
                .andExpect(jsonPath("$.code").value(StatusCode.NOT_FOUND))
                .andExpect(jsonPath("$.message").value("Could not find an artifact with ID 1250808601744904196"))
    ...
    void testCreateArtifactSuccess() throws Exception {
        //Given
        ArtifactDTO newArtifact = new ArtifactDTO(null, "Remembrall",
    ...
    void testUpdateArtifactSuccess() throws Exception {
        //Given
        ArtifactDTO newArtifact = new ArtifactDTO("1250808601744904197", "Remembrall",
                "new desc",
                "ImageUrl", null);
        ...
        updatedArtifact.setDescription("A new desc");
    ...
    void testUpdateArtifactNotFound() throws Exception {
        ...
        this.mockMvc.perform(put("/api/v1/artifacts/1250808601744904197").contentType(MediaType.APPLICATION_JSON).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(false))
                .andExpect(jsonPath("$.code").value(StatusCode.NOT_FOUND))
                .andExpect(jsonPath("$.message").value("Could not find an artifact with ID 1250808601744904197"))
                .andExpect(jsonPath("$.data").isEmpty());
    ...
    void testDeleteArtifactSuccess() throws Exception {
        //Given
        doNothing().when(this.artifactService).deleteArtifact("1250808601744904191");
    ...
    void testDeleteArtifactNotFound() throws Exception {
        ...
        this.mockMvc.perform(delete("/api/v1/artifacts/1250808601744904191").accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(false))
                .andExpect(jsonPath("$.code").value(StatusCode.NOT_FOUND))
                .andExpect(jsonPath("$.message").value("Could not find an artifact with ID 1250808601744904191"))
                .andExpect(jsonPath("$.data").isEmpty());
    }
